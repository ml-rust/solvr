...
impl<R: Runtime> PchipInterpolator<R> {
        ...
        C: TensorOps<R> + ScalarOps<R> + CompareOps<R> + RuntimeClient<R>,
    {
        ...
        // Compute slopes using Fritsch-Carlson method with tensor ops
        let slopes = compute_pchip_slopes(client, &validated.x, &validated.y)?;
...
mod tests {
    ...
    #[test]
    fn test_pchip_out_of_bounds_clamps() {
        let (device, client) = setup();
        ...
        let result = pchip.evaluate(&client, &x_oob).unwrap();
